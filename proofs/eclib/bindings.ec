...
from Jasmin require import JModel_x86.
from JazzEC require import Array4 Array5 Array6 Array7 Array8 Array9 Array16 Array24 Array25 Array32 Array48 Array128 Array160 Array256 Array384 Array768 Array960 Array1088 Array1024 Array1152 Array1408 Array1536 WArray1568 Array1410 Array1 Array2 Array640 Array320 Array416.
from JazzEC require import WArray16 WArray32 WArray128 WArray160 WArray512 WArray960 WArray2048 WArray1536 Array1568 WArray1410 WArray384 WArray1088 WArray1024 WArray640 WArray320 WArray416.
...
have ? : 2^4 = 16 by rewrite /=.
by smt(bs2int_range mem_range W4.size_w2bits).
qed.
realize touintP by smt().
realize gt0_size by done.      

bind op [W4.t & bool] W4."_.[_]" "get".
realize le_size by auto.
realize eq1_size by auto.
realize bvgetP by auto.

bind op [bool & W4.t] W4.init "init".
...
end W12. export W12 W12.ALU W12.SHIFT.
...
theory W13.
abbrev [-printing] size = 13.
clone include BitWordSH with op size <- size
rename "_XX" as "_13"
proof gt0_size by done,
size_le_256 by done.

end W13. export W13 W13.ALU W13.SHIFT.

bind bitstring W13.w2bits W13.bits2w W13.to_uint W13.to_sint W13.of_int W13.t 13.
realize size_tolist by auto.
realize tolistP by auto.
realize oflistP by smt(W13.bits2wK). 
realize ofintP by move => *;rewrite /of_int int2bs_mod.
realize tosintP. move => bv /=;rewrite /to_sint /smod /BVA_Top_W13_t.msb.
have -> /=: nth false (w2bits bv) (13 - 1) = 2 ^ (13 - 1) <= to_uint bv; last by smt().
rewrite /to_uint. 
rewrite -{2}(cat_take_drop 10 (w2bits bv)).
rewrite bs2int_cat size_take 1:// W13.size_w2bits /=.
rewrite -bs2int_div 1:// /= get_to_uint /=.
rewrite -bs2int_mod 1:// /= /to_uint.
have ? : 2^13 = 8192 by rewrite /=.
by smt(bs2int_range mem_range W13.size_w2bits).
qed.
realize touintP by smt().
realize gt0_size by done.


bind op [W13.t & bool] W13."_.[_]" "get".
realize le_size by auto.
realize eq1_size by auto.
realize bvgetP by auto.

bind op [bool & W13.t] W13.init "init".
realize size_1 by auto.
realize bvinitP.
move => f.  apply (eq_from_nth false).
+ rewrite size_w2bits (size_flatten' 1). 
...
theory W20.
abbrev [-printing] size = 20.
clone include BitWordSH with op size <- size
rename "_XX" as "_20"
proof gt0_size by done,
size_le_256 by done.

end W20. export W20 W20.ALU W20.SHIFT.


bind bitstring W20.w2bits W20.bits2w W20.to_uint W20.to_sint W20.of_int W20.t 20.
realize size_tolist by auto.
realize tolistP by auto.
realize oflistP by smt(W20.bits2wK). 
realize ofintP by move => *;rewrite /of_int int2bs_mod.
realize tosintP. move => bv /=;rewrite /to_sint /smod /BVA_Top_W20_t.msb.
have -> /=: nth false (w2bits bv) (20 - 1) = 2 ^ (20 - 1) <= to_uint bv; last by smt().
rewrite /to_uint. 
rewrite -{2}(cat_take_drop 10 (w2bits bv)).
rewrite bs2int_cat size_take 1:// W20.size_w2bits /=.
rewrite -bs2int_div 1:// /= get_to_uint /=.
rewrite -bs2int_mod 1:// /= /to_uint.
have ? : 2^20 = 1048576 by rewrite /=.
by smt(bs2int_range mem_range W20.size_w2bits).
qed.
realize touintP by smt().
realize gt0_size by done.


bind op [W20.t & bool] W20."_.[_]" "get".
realize le_size by auto.
realize eq1_size by auto.
realize bvgetP by auto.

bind op [bool & W20.t] W20.init "init".
realize size_1 by auto.
realize bvinitP.
move => f.  apply (eq_from_nth false).
+ rewrite size_w2bits (size_flatten' 1). 
...
op zeroextu13_32(a : W13.t) : W32.t =
  W32.of_int (to_uint a).

bind op [W13.t & W32.t] zeroextu13_32 "zextend".
realize bvzextendP.
have ? : 2^13 = 8192 by auto.
 by move => bv; rewrite /zeroextu32 /= of_uintK /=; smt(W13.to_uint_cmp).
qed.
realize le_size by done.
...
op truncateu_32_20(x : W32.t) : W20.t = W20.of_int (to_uint x).
bind op [W32.t & W20.t] truncateu_32_20 "truncate".
realize bvtruncateP.
move => mv; rewrite /truncateu_32_20 /W32.w2bits take_mkseq 1:// /= /w2bits.
apply (eq_from_nth witness);1: by smt(size_mkseq).
move => i; rewrite size_mkseq /= /max /= => ib.
rewrite !nth_mkseq 1..2:// /of_int /to_uint /= get_bits2w 1:// 
        nth_mkseq 1:// /= get_to_uint 1:// /= /to_uint /=.
have -> /=: (0 <= i && i < 32) by smt().
pose a := bs2int (w2bits mv). 
rewrite {1}(divz_eq a (2^(20-i)*2^i)) !mulrA divzMDl;
   1: by smt(StdOrder.IntOrder.expr_gt0).
rewrite dvdz_modzDl; 1: by
 have ->  : 2^(20-i) = 2^((20-i-1)+1); [ by smt() |
    rewrite exprS 1:/#; smt(dvdz_mull dvdz_mulr)].  
by have -> : (2 ^ (20 - i) * 2 ^ i) = 1048576; 
  [ rewrite -StdBigop.Bigint.Num.Domain.exprD_nneg 
     1,2:/# /= -!addrA /= | done ].
qed.
realize le_size by done.

op truncateu_32_13(x : W32.t) : W13.t = W13.of_int (to_uint x).
bind op [W32.t & W13.t] truncateu_32_13 "truncate".
realize bvtruncateP.
move => mv; rewrite /truncateu_32_13 /W32.w2bits take_mkseq 1:// /= /w2bits.
apply (eq_from_nth witness);1: by smt(size_mkseq).
move => i; rewrite size_mkseq /= /max /= => ib.
rewrite !nth_mkseq 1..2:// /of_int /to_uint /= get_bits2w 1:// 
        nth_mkseq 1:// /= get_to_uint 1:// /= /to_uint /=.
have -> /=: (0 <= i && i < 32) by smt().
pose a := bs2int (w2bits mv). 
rewrite {1}(divz_eq a (2^(13-i)*2^i)) !mulrA divzMDl;
   1: by smt(StdOrder.IntOrder.expr_gt0).
rewrite dvdz_modzDl; 1: by
 have ->  : 2^(13-i) = 2^((13-i-1)+1); [ by smt() |
    rewrite exprS 1:/#; smt(dvdz_mull dvdz_mulr)].  
by have -> : (2 ^ (13 - i) * 2 ^ i) = 8192; 
  [ rewrite -StdBigop.Bigint.Num.Domain.exprD_nneg 
     1,2:/# /= -!addrA /= | done ].
qed.
realize le_size by done.

op truncateu_32_12(x : W32.t) : W12.t = W12.of_int (to_uint x).
bind op [W32.t & W12.t] truncateu_32_12 "truncate".
realize bvtruncateP.
move => mv; rewrite /truncateu_32_12 /W32.w2bits take_mkseq 1:// /= /w2bits.
apply (eq_from_nth witness);1: by smt(size_mkseq).
move => i; rewrite size_mkseq /= /max /= => ib.
rewrite !nth_mkseq 1..2:// /of_int /to_uint /= get_bits2w 1:// 
        nth_mkseq 1:// /= get_to_uint 1:// /= /to_uint /=.
...
op truncateu_32_10(x : W32.t) : W10.t = W10.of_int (to_uint x).
...
bind op [W64.t & W256.t] W4u64.zeroextu256 "zextend".
realize bvzextendP.
...
bind op [W256.t & W32.t] W8u32.truncateu32 "truncate".
realize bvtruncateP.
move => mv; rewrite /truncateu32 /W256.w2bits take_mkseq 1:// /= /w2bits.
apply (eq_from_nth witness);1: by smt(size_mkseq).
move => i; rewrite size_mkseq /= /max /= => ib.
rewrite !nth_mkseq 1..2:// /of_int /to_uint /= get_bits2w 1:// 
        nth_mkseq 1:// /= get_to_uint /= /to_uint /=.
...
bind op [W256.t & W16.t] W16u16.truncateu16 "truncate".
realize bvtruncateP.
move => mv; rewrite /truncateu16 /W256.w2bits take_mkseq 1:// /= /w2bits.
apply (eq_from_nth witness);1: by smt(size_mkseq).
move => i; rewrite size_mkseq /= /max /= => ib.
rewrite !nth_mkseq 1..2:// /of_int /to_uint /= get_bits2w 1:// 
        nth_mkseq 1:// /= get_to_uint /= /to_uint /=.
...
bind array Array416."_.[_]" Array416."_.[_<-_]" Array416.to_list Array416.of_list Array416.t 416.
realize tolistP by done.
realize get_setP by smt(Array416.get_setE). 
realize eqP by smt(Array416.tP).
realize get_out by smt(Array416.get_out).
realize gt0_size by done.


bind array Array640."_.[_]" Array640."_.[_<-_]" Array640.to_list Array640.of_list Array640.t 640.
realize tolistP by done.
...
op init_array640_w8(f : int -> W8.t) = Array640.init f.
bind op [W8.t & Array640.t] init_array640_w8 "ainit".
realize bvainitP.
proof.
rewrite /init_array640_w8 => f.
rewrite BVA_Top_Array640_Array640_t.tolistP.
apply eq_in_mkseq => i i_bnd;
smt(Array640.initE).
qed.

op init_array416_w8(f : int -> W8.t) = Array416.init f.
bind op [W8.t & Array416.t] init_array416_w8 "ainit".
realize bvainitP.
proof.
rewrite /init_array416_w8 => f.
rewrite BVA_Top_Array416_Array416_t.tolistP.
apply eq_in_mkseq => i i_bnd;
smt(Array416.initE).
qed.


op init_array320_w8(f : int -> W8.t) = Array320.init f.
bind op [W8.t & Array320.t] init_array320_w8 "ainit".
realize bvainitP.
proof.
rewrite /init_array320_w8 => f.
rewrite BVA_Top_Array320_Array320_t.tolistP.
apply eq_in_mkseq => i i_bnd;
smt(Array320.initE).
qed.
...
op init_256_4 (f: int -> W4.t) : W4.t Array256.t = Array256.init f.

bind op [W4.t & Array256.t] init_256_4 "ainit".
realize bvainitP.
proof.
rewrite /init_256_4 => f.
rewrite BVA_Top_Array256_Array256_t.tolistP.
apply eq_in_mkseq => i i_bnd;
smt(Array256.initE).
qed.

op init_256_10 (f: int -> W10.t) : W10.t Array256.t = Array256.init f.

bind op [W10.t & Array256.t] init_256_10 "ainit".
realize bvainitP.
proof.
rewrite /init_256_10 => f.
rewrite BVA_Top_Array256_Array256_t.tolistP.
apply eq_in_mkseq => i i_bnd;
smt(Array256.initE).
qed.
...
op init_256_20 (f: int -> W20.t) : W20.t Array256.t = Array256.init f.

bind op [W20.t & Array256.t] init_256_20 "ainit".
realize bvainitP.
proof.
rewrite /init_256_20 => f.
rewrite BVA_Top_Array256_Array256_t.tolistP.
apply eq_in_mkseq => i i_bnd;
smt(Array256.initE).
qed.
...
op init_256_13 (f: int -> W13.t) : W13.t Array256.t = 
  Array256.init f.

bind op [W13.t & Array256.t] init_256_13 "ainit".
realize bvainitP.
rewrite /init_256_13 => f.
rewrite BVA_Top_Array256_Array256_t.tolistP.
apply eq_in_mkseq => i i_bnd;
smt(Array256.initE).
qed.
...
op sliceget640_8_128 (arr: W8.t Array640.t) (offset: int) : W128.t = 
   if 8 %| offset then 
    get128_direct ((init8 (fun (i_0 : int) => arr.[i_0])))%WArray640 (offset %/ 8)
   else W128.bits2w (take 128 (drop offset (flatten (map W8.w2bits (to_list arr))))).

bind op [W8.t & W128.t & Array640.t] sliceget640_8_128 "asliceget".
realize bvaslicegetP.
move => /= arr offset; rewrite /sliceget640_8_128 /= => H k kb. 
case (8%| offset) => /= *; last by smt(W128.get_bits2w).
rewrite /get128_direct pack16E initiE 1:/# /= initiE 1:/# /= initiE 1:/# /=.
rewrite nth_take 1,2:/# nth_drop 1,2:/#.
rewrite (BitEncoding.BitChunking.nth_flatten false 8 _). 
+ rewrite allP => x /=; rewrite mapP => He; elim He;smt(W8.size_w2bits).
rewrite (nth_map W8.zero []); 1: smt(Array640.size_to_list).
rewrite nth_mkseq 1:/# /= /#.
qed.
realize le_size by done.

op sliceget416_8_128 (arr: W8.t Array416.t) (offset: int) : W128.t = 
   if 8 %| offset then 
    get128_direct ((init8 (fun (i_0 : int) => arr.[i_0])))%WArray416 (offset %/ 8)
   else W128.bits2w (take 128 (drop offset (flatten (map W8.w2bits (to_list arr))))).

bind op [W8.t & W128.t & Array416.t] sliceget416_8_128 "asliceget".
realize bvaslicegetP.
move => /= arr offset; rewrite /sliceget416_8_128 /= => H k kb. 
case (8%| offset) => /= *; last by smt(W128.get_bits2w).
rewrite /get128_direct pack16E initiE 1:/# /= initiE 1:/# /= initiE 1:/# /=.
rewrite nth_take 1,2:/# nth_drop 1,2:/#.
rewrite (BitEncoding.BitChunking.nth_flatten false 8 _). 
+ rewrite allP => x /=; rewrite mapP => He; elim He;smt(W8.size_w2bits).
rewrite (nth_map W8.zero []); 1: smt(Array416.size_to_list).
rewrite nth_mkseq 1:/# /= /#.
qed.
realize le_size by done.

op sliceget320_8_128 (arr: W8.t Array320.t) (offset: int) : W128.t = 
   if 8 %| offset then 
    get128_direct ((init8 (fun (i_0 : int) => arr.[i_0])))%WArray320 (offset %/ 8)
   else W128.bits2w (take 128 (drop offset (flatten (map W8.w2bits (to_list arr))))).

bind op [W8.t & W128.t & Array320.t] sliceget320_8_128 "asliceget".
realize bvaslicegetP.
move => /= arr offset; rewrite /sliceget320_8_128 /= => H k kb. 
case (8%| offset) => /= *; last by smt(W128.get_bits2w).
rewrite /get128_direct pack16E initiE 1:/# /= initiE 1:/# /= initiE 1:/# /=.
rewrite nth_take 1,2:/# nth_drop 1,2:/#.
rewrite (BitEncoding.BitChunking.nth_flatten false 8 _). 
+ rewrite allP => x /=; rewrite mapP => He; elim He;smt(W8.size_w2bits).
rewrite (nth_map W8.zero []); 1: smt(Array320.size_to_list).
rewrite nth_mkseq 1:/# /= /#.
qed.
realize le_size by done.
...
op sliceget256_32_256 (arr: W32.t Array256.t) (offset: int) : W256.t = 
   if 8 %| offset then 
    get256_direct ((init32 (fun (i_0 : int) => arr.[i_0])))%WArray1024 (offset %/ 8)
   else W256.bits2w (take 256 (drop offset (flatten (map W32.w2bits (to_list arr))))).

bind op [W32.t & W256.t & Array256.t] sliceget256_32_256 "asliceget".
realize bvaslicegetP.
move => /= arr offset; rewrite /sliceget256_32_256 /= => H k kb. 
case (8%| offset) => /= *; last by smt(W256.get_bits2w).
rewrite /get256_direct pack32E initiE 1:/# /= initiE 1:/# /= initiE 1:/# /= bits8E initiE 1:/# /=.
rewrite nth_take 1,2:/# nth_drop 1,2:/#.
rewrite (BitEncoding.BitChunking.nth_flatten false 32 _). 
+ rewrite allP => x /=; rewrite mapP => He; elim He;smt(W32.size_w2bits).
rewrite (nth_map W32.zero []); 1: smt(Array256.size_to_list).
by rewrite nth_mkseq /#.
qed.
realize le_size by done.
...
op sliceset256_32_256 (arr: W32.t Array256.t) (offset: int) (bv: W256.t) : W32.t Array256.t =
  if 8 %| offset
  then (init (fun (i3 : int) => get32 (set256_direct ((init32 (fun (i_0 : int) => arr.[i_0])))%WArray1024 (offset %/ 8) bv) i3))%Array256
  else  Array256.of_list witness (map W32.bits2w (chunk 32 (take offset (flatten (map W32.w2bits (to_list arr))) ++ w2bits bv ++
  drop (offset + 256) (flatten (map W32.w2bits (to_list arr)))))).

lemma size_flatten_W32_w2bits (a : W32.t list) :
  (size (flatten (map W32.w2bits (a))))  =  32 * size a.
proof.
  rewrite size_flatten -map_comp /(\o) /=.
  rewrite StdBigop.Bigint.sumzE /= StdBigop.Bigint.BIA.big_mapT /(\o) /=. 
  rewrite StdBigop.Bigint.big_constz count_predT /#.
...
bind op [W32.t & W256.t & Array256.t] sliceset256_32_256 "asliceset".
realize bvaslicesetP. 
move => arr offset bv H /= k kb; rewrite /sliceset256_32_256 /=. 
case (8 %| offset) => /= *; last first.
+ rewrite of_listK; 1: by rewrite size_map size_chunk 1:// !size_cat size_take;
      by smt(size_take size_drop  W32.size_w2bits size_cat Array256.size_to_list size_flatten_W32_w2bits size_ge0). 
  rewrite -(map_comp W32.w2bits W32.bits2w) /(\o). 
  have := eq_in_map ((fun (x : bool list) => w2bits ((bits2w x))%W32)) idfun (chunk 32
        (take offset (flatten (map W32.w2bits (to_list arr))) ++ w2bits bv ++
         drop (offset + 256) (flatten (map W32.w2bits (to_list arr))))).
  rewrite iffE => [#] -> * /=; 1: by smt(in_chunk_size W32.bits2wK).
  rewrite map_id /= chunkK 1://;1: by rewrite !size_cat size_take;
    by smt(size_take size_drop  W32.size_w2bits size_cat Array256.size_to_list size_flatten_W32_w2bits size_ge0). 
  by rewrite !nth_cat !size_cat /=;
     smt(nth_take nth_drop size_take size_drop  W32.size_w2bits size_cat Array256.size_to_list size_flatten_W32_w2bits size_ge0).
     
rewrite (nth_flatten _ 32); 1: by rewrite allP => i;rewrite mapP => He; elim He;smt(W32.size_w2bits).
rewrite (nth_map W32.zero []); 1: smt(Array256.size_to_list).
rewrite nth_mkseq 1:/# /= initiE 1:/# /= get32E pack4E initiE 1:/# /= initiE 1:/# /= /set256_direct.
rewrite initiE 1:/# /=.
case (offset <= k && k < offset + 256) => *; 1: by 
  rewrite ifT 1:/#  get_bits8 /= 1,2:/# initiE // initiE //.
rewrite ifF 1:/# initiE 1:/# /=.
...
op sliceset16_8_128 (arr: W8.t Array16.t) (offset: int) (bv: W128.t) : W8.t Array16.t = 
  if 8 %| offset
  then Array16.init (fun (i3 : int) => get8 (set128_direct ((init8 (fun (i_0 : int) => arr.[i_0])))%WArray16 (offset %/ 8) bv) i3)
  else  Array16.of_list witness (map W8.bits2w (chunk 8 (take offset (flatten (map W8.w2bits (to_list arr))) ++ w2bits bv ++
  drop (offset + 128) (flatten (map W8.w2bits (to_list arr)))))).


bind op [W8.t & W128.t & Array16.t] sliceset16_8_128 "asliceset".
realize bvaslicesetP.
move => arr offset bv H /= k kb; rewrite /sliceset16_8_128 /=. 
case (8 %| offset) => /= *; last first.
+ rewrite of_listK; 1: by  rewrite size_map size_chunk 1:// !size_cat size_take; 
      by smt(size_take size_drop  W8.size_w2bits size_cat Array16.size_to_list size_flatten_W8_w2bits size_ge0). 
  rewrite -(map_comp W8.w2bits W8.bits2w) /(\o). 
  have := eq_in_map ((fun (x : bool list) => w2bits ((bits2w x))%W8)) idfun (chunk 8
        (take offset (flatten (map W8.w2bits (to_list arr))) ++ w2bits bv ++
         drop (offset + 128) (flatten (map W8.w2bits (to_list arr))))).
...
op sliceset416_8_128 (arr: W8.t Array416.t) (offset: int) (bv: W128.t) : W8.t Array416.t = 
  if 8 %| offset
  then Array416.init
     (WArray416.get8
        (set128_direct (WArray416.init8 (fun (i_0 : int) => arr.[i_0])) (
           offset %/ 8) bv))
  else  Array416.of_list witness (map W8.bits2w (chunk 8 (take offset (flatten (map W8.w2bits (to_list arr))) ++ w2bits bv ++
  drop (offset + 128) (flatten (map W8.w2bits (to_list arr)))))).

bind op [W8.t & W128.t & Array416.t] sliceset416_8_128 "asliceset".
realize bvaslicesetP.
move => arr offset bv H /= k kb; rewrite /sliceset416_8_128 /=. 
case (8 %| offset) => /= *; last first.
+ rewrite of_listK; 1: by  rewrite size_map size_chunk 1:// !size_cat size_take; 
      by smt(size_take size_drop  W8.size_w2bits size_cat Array416.size_to_list size_flatten_W8_w2bits size_ge0). 
  rewrite -(map_comp W8.w2bits W8.bits2w) /(\o). 
  have := eq_in_map ((fun (x : bool list) => w2bits ((bits2w x))%W8)) idfun (chunk 8
        (take offset (flatten (map W8.w2bits (to_list arr))) ++ w2bits bv ++
         drop (offset + 128) (flatten (map W8.w2bits (to_list arr))))).
  ...
  rewrite map_id /= chunkK 1://;1: by rewrite !size_cat size_take;
    by smt(size_take size_drop  W8.size_w2bits size_cat Array416.size_to_list size_flatten_W8_w2bits size_ge0). 
  by rewrite !nth_cat !size_cat /=;
     smt(nth_take nth_drop size_take size_drop  W8.size_w2bits size_cat Array416.size_to_list size_flatten_W8_w2bits size_ge0). 
rewrite (nth_flatten _ 8); 1: by rewrite allP => i;rewrite mapP => He; elim He;smt(W8.size_w2bits).
rewrite (nth_map W8.zero []); 1: smt(Array416.size_to_list).
rewrite nth_mkseq 1:/# /= initiE 1:/# /= /get8 /set128_direct /pack2_t.
rewrite initiE 1:/# /= initiE 1:/# /=.
case (offset <= k && k < offset + 128) => *. 
  by rewrite ifT 1:/#  get_bits8 /= 1,2:/# initiE // initiE //.
rewrite ifF 1:/#.
rewrite (nth_flatten _ 8); 1: by rewrite allP => i;rewrite mapP => He; elim He;smt(W8.size_w2bits).
rewrite (nth_map W8.zero []); 1: smt(Array416.size_to_list).
...
op sliceset640_8_128 (arr: W8.t Array640.t) (offset: int) (bv: W128.t) : W8.t Array640.t = 
  if 8 %| offset
  then Array640.init
     (WArray640.get8
        (set128_direct (WArray640.init8 (fun (i_0 : int) => arr.[i_0])) (
           offset %/ 8) bv))
  else  Array640.of_list witness (map W8.bits2w (chunk 8 (take offset (flatten (map W8.w2bits (to_list arr))) ++ w2bits bv ++
  drop (offset + 128) (flatten (map W8.w2bits (to_list arr)))))).

bind op [W8.t & W128.t & Array640.t] sliceset640_8_128 "asliceset".
realize bvaslicesetP.
move => arr offset bv H /= k kb; rewrite /sliceset640_8_128 /=. 
case (8 %| offset) => /= *; last first.
+ rewrite of_listK; 1: by  rewrite size_map size_chunk 1:// !size_cat size_take; 
      by smt(size_take size_drop  W8.size_w2bits size_cat Array640.size_to_list size_flatten_W8_w2bits size_ge0). 
  rewrite -(map_comp W8.w2bits W8.bits2w) /(\o). 
  have := eq_in_map ((fun (x : bool list) => w2bits ((bits2w x))%W8)) idfun (chunk 8
        (take offset (flatten (map W8.w2bits (to_list arr))) ++ w2bits bv ++
         drop (offset + 128) (flatten (map W8.w2bits (to_list arr))))).
  ...
  rewrite map_id /= chunkK 1://;1: by rewrite !size_cat size_take;
    by smt(size_take size_drop  W8.size_w2bits size_cat Array640.size_to_list size_flatten_W8_w2bits size_ge0). 
  by rewrite !nth_cat !size_cat /=;
     smt(nth_take nth_drop size_take size_drop  W8.size_w2bits size_cat Array640.size_to_list size_flatten_W8_w2bits size_ge0). 
rewrite (nth_flatten _ 8); 1: by rewrite allP => i;rewrite mapP => He; elim He;smt(W8.size_w2bits).
rewrite (nth_map W8.zero []); 1: smt(Array640.size_to_list).
rewrite nth_mkseq 1:/# /= initiE 1:/# /= /get8 /set128_direct /pack2_t.
rewrite initiE 1:/# /= initiE 1:/# /=.
...
op sliceset320_8_128 (arr: W8.t Array320.t) (offset: int) (bv: W128.t) : W8.t Array320.t = 
  if 8 %| offset
  then Array320.init
     (WArray320.get8
        (set128_direct (WArray320.init8 (fun (i_0 : int) => arr.[i_0])) (
           offset %/ 8) bv))
  else  Array320.of_list witness (map W8.bits2w (chunk 8 (take offset (flatten (map W8.w2bits (to_list arr))) ++ w2bits bv ++
  drop (offset + 128) (flatten (map W8.w2bits (to_list arr)))))).

bind op [W8.t & W128.t & Array320.t] sliceset320_8_128 "asliceset".
realize bvaslicesetP.
move => arr offset bv H /= k kb; rewrite /sliceset320_8_128 /=. 
case (8 %| offset) => /= *; last first.
+ rewrite of_listK; 1: by  rewrite size_map size_chunk 1:// !size_cat size_take; 
      by smt(size_take size_drop  W8.size_w2bits size_cat Array320.size_to_list size_flatten_W8_w2bits size_ge0). 
  rewrite -(map_comp W8.w2bits W8.bits2w) /(\o). 
  have := eq_in_map ((fun (x : bool list) => w2bits ((bits2w x))%W8)) idfun (chunk 8
        (take offset (flatten (map W8.w2bits (to_list arr))) ++ w2bits bv ++
         drop (offset + 128) (flatten (map W8.w2bits (to_list arr))))).
  ...
  rewrite map_id /= chunkK 1://;1: by rewrite !size_cat size_take;
    by smt(size_take size_drop  W8.size_w2bits size_cat Array320.size_to_list size_flatten_W8_w2bits size_ge0). 
  by rewrite !nth_cat !size_cat /=;
     smt(nth_take nth_drop size_take size_drop  W8.size_w2bits size_cat Array320.size_to_list size_flatten_W8_w2bits size_ge0). 
rewrite (nth_flatten _ 8); 1: by rewrite allP => i;rewrite mapP => He; elim He;smt(W8.size_w2bits).
rewrite (nth_map W8.zero []); 1: smt(Array320.size_to_list).
rewrite nth_mkseq 1:/# /= initiE 1:/# /= /get8 /set128_direct /pack2_t.
rewrite initiE 1:/# /= initiE 1:/# /=.
