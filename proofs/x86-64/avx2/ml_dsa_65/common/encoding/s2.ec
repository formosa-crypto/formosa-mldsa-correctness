...
(*
   For s2:
   - Input: 6 polynomials × 256 W32.t coefficients = 1536 W32.t elements (6144 bytes)
   - Output: 1536 coefficients × 4 bits = 6144 bits = 768 bytes
*)
...
lemma s2_encode _a :
    ...
    hoare [ M.s2____encode :
       s2 = _a /\ wpolylvec_srng (input_unflatten _a) Eta Eta
     ==>
...
move => Hkvec.
proc => /=.
while (0 <= i <= 6 /\ s2 = _a /\ wpolylvec_srng (input_unflatten _a) Eta Eta  /\
       forall k, 0 <= k < i =>
...
(*
   For s2 decode:
   - Input: 768 bytes = Array768 W8.t
   - Output: 6 polynomials × 256 W32.t coefficients = 1536 W32.t elements
*)

op  decode_input_unflatten(a : 'a Array768.t) =
     LArray.init (fun i => Array128.of_list witness (sub a (128*i) 128)).
op  decode_output_unflatten(a : 'a Array1536.t) =
     LArray.init (fun i => Array256.of_list witness (sub a (256*i) 256)).
...
while (0 <= i <= 6 /\ encoded = _a /\
       forall k, 0 <= k < i =>
       (lifts_wpolylvec (decode_output_unflatten s2)).[k] =
...
+ have -> : (lifts_wpolylvec
   (decode_output_unflatten
    ...
    (lifts_wpolylvec (decode_output_unflatten s2{hr})).[k]; last by smt().
